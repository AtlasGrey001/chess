...
public class ChessGame {
    ...
    public boolean sameMove(ChessMove one, ChessMove two){
        if(one.getStartPosition().getRow()!=two.getStartPosition().getRow()){
            return false;
        }
        if(one.getStartPosition().getColumn()!=two.getStartPosition().getColumn()){
            return false;
        }
        if(one.getEndPosition().getRow()!=two.getEndPosition().getRow()){
            return false;
        }
        if(one.getEndPosition().getColumn()!=two.getEndPosition().getColumn()){
            return false;
        }
    ...
    public void castleMove(Collection<ChessMove> moves,ChessBoard board,ChessPosition pos,int num){
        ...
        for(int b=c+dir;b!=rpos.getColumn();b+=dir){
            if(board.getPiece(new ChessPosition(r,b))!=null){
                return;
            }
        }
